<template>
    <div class="col-md-8 col-md-offset-2" style="padding-top:160px;padding-bottom:30px">
        <h1 class="text-center" style="font-weight:700;font-size:35px; font-family: 'Noto Serif',serif;margin:0">Enjoy music from there!</h1>
...
export default {
  ...
  methods: {
    searchmusic() {
      this.isshow = true;
      axios
        .post("/findallsong", { content: this.value, offset: 0 })
        .then(response => {
          this.isshow = false;
    ...
    handleClick(newIndex) {
      axios
        .post("/findallsong", { content: this.value, offset: newIndex })
        .then(response => {
          this.songlist = response.data.result.songs;
